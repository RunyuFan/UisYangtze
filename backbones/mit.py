...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, H, W) -> Tensor:
        B, N, C = x.shape
        # print('before sr_ratio x shape', x.shape)
        ...
        # print('after sr_ratio x shape', x.shape)
        k, v = self.kv(x).reshape(B, -1, 2, self.head, C // self.head).permute(2, 0, 3, 1, 4)
        # print('k, v', x.shape, q.shape, k.shape, v.shape, (q @ k).shape, (self.kv(x).reshape(B, -1, 2, self.head, C // self.head).permute(2, 0, 3, 1, 4)).shape)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.proj(x)
        # print('x = self.proj(x):', x.shape)
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)
        # print('x.flatten(2).transpose(1, 2):', x.shape)
        x = self.norm(x)
        # print('self.norm(x):', x.shape)
        """
        x = self.proj(x): torch.Size([32, 64, 64, 64])
        x.flatten(2).transpose(1, 2): torch.Size([32, 4096, 64])
        self.norm(x): torch.Size([32, 4096, 64])
        x = self.proj(x): torch.Size([32, 128, 32, 32])
        x.flatten(2).transpose(1, 2): torch.Size([32, 1024, 128])
        self.norm(x): torch.Size([32, 1024, 128])
        x = self.proj(x): torch.Size([32, 320, 16, 16])
        x.flatten(2).transpose(1, 2): torch.Size([32, 256, 320])
        self.norm(x): torch.Size([32, 256, 320])
        x = self.proj(x): torch.Size([32, 512, 8, 8])
        x.flatten(2).transpose(1, 2): torch.Size([32, 64, 512])
        self.norm(x): torch.Size([32, 64, 512])
        """
        return x, H, W
...
class MiT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        B = x.shape[0]
        # print('patch_embed1 x shape:', x.shape)
        # stage 1
        ...
        # print('patch_embed1 x1 shape:', x1.shape)
        # stage 2
        ...
        # stage 3
        # print('patch_embed1 x2 shape:', x2.shape)
