from dataset_CJZY_Semantic import UVdataset
import argparse
...
# from model_fuse import SemiformerNet
from loss import DiceCELoss, RecallCrossEntropy, FocalLoss, IoULoss
...
def main(args):
    ...
    # train_dataset = UVdataset(txt='.\\data\\testUV_Semantic_CJZY.txt',transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))
    # # val_dataset=OSSSDateSet('.\\data\\val_1_10.txt')
    # test_dataset = UVdataset(txt='.\\data\\trainUV_Semantic_CJZY.txt',transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))
    train_dataset = UVdataset(txt='data\\trainUV_CJZY_0.5.txt',transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))
    # val_dataset=OSSSDateSet('.\\data\\val_1_10.txt')
    ...
    cost2 = FocalLoss().to(device) # FocalLoss() DiceCELoss() OhemCrossEntropy().to(device)  # nn.CrossEntropyLoss().to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            classes = ['非城中村', '城中村'] # ('住宅区', '公共服务区域', '商业区', '城市绿地', '工业区')  'Mask',
        ...
        if  miou > miou_max:
            ...
            torch.save(model2, os.path.join(args.model_path, 'UV-Semantic_CJZY-semiformer-0.5#1.pth'))
            miou_max = miou
            best_epoch = epoch
        # if acc_3 > best_acc_3:
    ...
    # print('save new best acc_1', best_acc_1)
        print('best miou', miou_max, best_epoch)
