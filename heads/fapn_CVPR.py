...
class FSCBAM(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.conv_atten = CBAM(c1)
        # self.csa_layer = csa_layer(c2, 1)
        self.conv = nn.Conv2d(c1, c2, 1, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # feat = torch.mul(x, atten)
        # x = x + feat
        return  self.conv(atten)

class FAMCBAM(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.lateral_conv = FSCBAM(c1, c2)
...
class ChannelSelfAtt(nn.Module):
...
class FaPNCBAMHead(nn.Module):
    def __init__(self, in_channels, channel=128, num_classes=19):
        ...
        for ch in in_channels[1:]:
            self.align_modules.append(FAMCBAM(ch, channel))
            self.output_convs.append(ConvModule(channel, channel, 3, 1, 1))
...
class FaPN(nn.Module):
    def __init__(self, in_channels, channel=128, num_classes=19):
        ...
        self.head = FaPNHead(in_channels, channel, num_classes)
...
class FaPN_Fuse(nn.Module):
    ...
    def __init__(self, in_channels, channel=128, num_classes=19):
        ...
        self.head_hrs = FaPNHead(in_channels, channel, 64)
        self.head = FaPNHead(in_channels, channel, 64)
